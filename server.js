...
// Confia no proxy do Render para o Rate Limiter não bloquear toda a gente
app.set('trust proxy', 1);

// === ATIVAÇÃO DOS ESCUDOS ===
app.use(helmet({ contentSecurityPolicy: false })); // Protege headers, mas não bloqueia iFrames de jogos
app.use(mongoSanitize()); // Protege contra injeções no banco de dados
app.use(cors());
app.use(express.json());
app.use(express.static('public'));

// Rate Limiter para rotas sensíveis
const loginLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutos
    max: 50, // Limite generoso para não o bloquear durante os testes
...
async function initializeAIBot() {
    ...
    } catch(e) { console.error("Erro ao iniciar Bot", e); }
}
...
app.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        res.json({ 
            token, myId: user._id, email: user.email, displayName: user.displayName, 
            photoUrl: user.photoUrl, sectors: user.sectors, theme: user.theme, 
            fontSize: user.fontSize, notificationSound: user.notificationSound, 
            xp: user.xp, level: user.level, dailyMessagesSent: user.dailyMessagesSent, 
            dailyMissionCompleted: user.dailyMissionCompleted, lastActiveDate: user.lastActiveDate, 
...
app.post('/add-xp', async (req, res) => {
    try {
        ...
        if(!userId) return res.status(400).json({ error: 'Sem ID' });
...
app.put('/settings', async (req, res) => {
    try {
        ...
        if(req.body.sectors !== undefined) u.sectors = req.body.sectors;
        if(req.body.displayName !== undefined) u.displayName = req.body.displayName;
        if(req.body.photoUrl !== undefined) u.photoUrl = req.body.photoUrl;
        if(req.body.phone !== undefined) u.phone = req.body.phone;
        if(req.body.bio !== undefined) u.bio = req.body.bio;
        if(req.body.chatWallpaper !== undefined) u.chatWallpaper = req.body.chatWallpaper;
        if(req.body.fontSize !== undefined) u.fontSize = req.body.fontSize;
        if(req.body.notificationSound !== undefined) u.notificationSound = req.body.notificationSound;
        await u.save();
        res.json(u);
    } catch (e) {
        res.status(500).json({error: 'Erro interno'});
...
app.post('/block-user', async (req, res) => {
    try {
        ...
        if(user && !user.blockedUsers.includes(req.body.targetId)) {
...
// Rotas Base Restauradas e Tratadas
...
app.get('/messages/:myId/:otherId', async (req, res) => { try { res.json(await Message.find({ $or: [ { sender: req.params.myId, receiver: req.params.otherId }, { sender: req.params.otherId, receiver: req.params.myId } ] }).sort('timestamp')); } catch (e) { res.status(500).json([]); } });
...
app.get('/group-messages/:groupId', async (req, res) => { try { res.json(await Message.find({ groupId: req.params.groupId }).populate('sender', 'displayName photoUrl').sort('timestamp')); } catch (e) { res.status(500).json([]); } });
...
setInterval(async () => {
    try {
        ...
        for (const s of pendings) {
            ...
            await msg.save();
            if (s.isGroup) { io.to(s.targetId).emit('receive_message', msg); } 
            else { const rSocket = users[s.targetId]; if (rSocket) io.to(rSocket).emit('receive_message', msg); const sSocket = users[s.senderId]; if (sSocket) io.to(sSocket).emit('receive_message', msg); }
        }
    } catch(e) {}
}, 10000); 
