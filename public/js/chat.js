...
window.openBulkCreateGroupModal = function() {
    const usersOnly = selectedActionContacts.filter(c => !c.isGroup).map(c => c.id);
    if (usersOnly.length === 0) return alert("Selecione pelo menos um contato para criar um grupo.");
    clearContactSelection();
    openCreateGroupModal(usersOnly);
};

window.openBulkScheduleModal = function() {
    const validContacts = selectedActionContacts;
    if(validContacts.length === 0) return;
    window.bulkSchedulingContacts = [...validContacts];
    if (typeof showElement === 'function') { showElement('schedule-modal'); } else { document.getElementById('schedule-modal').classList.remove('hidden'); }
    const selectEl = document.getElementById('schedule-target');
    selectEl.innerHTML = `<option value="bulk">Envio em Massa (${validContacts.length} contatos)</option>`;
    selectEl.value = 'bulk'; selectEl.disabled = true; 
    document.getElementById('schedule-datetime').value = ''; document.getElementById('schedule-text').value = '';
    document.querySelector('#schedule-modal .chic-btn').setAttribute('onclick', 'executeBulkSchedule()');
};

window.executeBulkSchedule = async function() {
    const dt = document.getElementById('schedule-datetime').value; const txt = document.getElementById('schedule-text').value.trim();
    if(!dt || !txt) return alert("Preencha a data e a mensagem!");
    const btn = document.querySelector('#schedule-modal .chic-btn'); btn.innerHTML = 'Agendando...';
    try {
        for(let contact of window.bulkSchedulingContacts) {
            await fetch('/schedule-message', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ senderId: myId, targetId: contact.id, isGroup: contact.isGroup, content: txt, scheduledTime: new Date(dt).toISOString() }) });
        }
        alert(`Mensagem programada para ${window.bulkSchedulingContacts.length} contato(s) com sucesso!`);
        closeScheduleModal(); clearContactSelection();
    } catch(e) { alert("Erro ao agendar as mensagens."); } finally { btn.innerHTML = 'Agendar'; }
};

window.closeScheduleModal = function() {
    if (typeof hideElement === 'function') hideElement('schedule-modal'); else document.getElementById('schedule-modal').classList.add('hidden');
    document.getElementById('schedule-target').disabled = false;
    document.querySelector('#schedule-modal .chic-btn').setAttribute('onclick', 'saveScheduledMessage()');
};
...
window.changeEmojiCategory = function(categoryName, element) {
    const picker = document.getElementById('neo-emoji-picker'); if (!picker) return;
    ...
    document.querySelectorAll('.category-icon').forEach(icon => icon.classList.remove('active')); if(element) element.classList.add('active');
};

setTimeout(() => { 
    const picker = document.getElementById('neo-emoji-picker'); const msgInput = document.getElementById('message-input'); 
    if (picker && msgInput) { 
        picker.addEventListener('emoji-click', event => { 
            if (window.isReactingToMsgId) { socket.emit('react_message', { msgId: window.isReactingToMsgId, emoji: event.detail.unicode, receiverId: currentChatId, groupId: isGroupChat ? currentChatId : null }); window.isReactingToMsgId = null; document.getElementById('emoji-drawer').style.height = '0px'; return; }
            msgInput.innerText += event.detail.unicode; emitTypingStatus('typing'); 
            const dynamicActionIcon = document.getElementById('dynamic-action-icon'); if(dynamicActionIcon && dynamicActionIcon.innerText !== 'send' && dynamicActionIcon.innerText !== 'check') { dynamicActionIcon.innerText = 'send'; } 
        }); 
    } 
    if(msgInput) { msgInput.addEventListener('focus', () => { const drawer = document.getElementById('emoji-drawer'); if (drawer && drawer.style.height === '300px') { drawer.style.height = '0px'; } }); }
    document.addEventListener('click', (e) => { if (!e.target.closest('#emoji-drawer') && !e.target.closest('#btn-emoji')) { const drawer = document.getElementById('emoji-drawer'); if (drawer && drawer.style.height === '300px') drawer.style.height = '0px'; } }); 
}, 1000);
...
window.startRecording = async function() { 
    ...
    try { 
        ...
        const slideCancel = document.getElementById('slide-to-cancel-ui'); if(slideCancel) slideCancel.classList.add('hidden');
        const slideLock = document.getElementById('slide-to-lock-ui'); if(slideLock) slideLock.classList.add('hidden');
...
socket.on('online_users', (list) => { 
    ...
    if (currentChatId && !isGroupChat) { 
        ...
        if (headerDot) { headerDot.style.display = 'block'; headerDot.className = `status-dot ${isOnline ? 'status-online' : 'status-offline'}`; }
        if (headerText) { headerText.innerText = isOnline ? 'Online' : 'Offline'; headerText.style.color = isOnline ? '#10B981' : '#EF4444'; }
    } 
});
...
socket.on('message_edited', (data) => {
    ...
    if (el) { const textSpan = el.querySelector('.msg-text-content'); if (textSpan) textSpan.innerHTML = escapeHTML(data.newContent) + ' <span style="font-size:10.5px; opacity:0.7; margin-left: 5px;">(editado)</span>'; }
    if (messageCache[currentChatId]) { const cMsg = messageCache[currentChatId].find(m => m._id === data.msgId); if (cMsg) cMsg.content = data.newContent + ' (editado)'; const cacheKey = isGroupChat ? `chat_cache_group_${currentChatId}` : `chat_cache_${currentChatId}`; localStorage.setItem(cacheKey, JSON.stringify(messageCache[currentChatId].slice(-50))); }
});

socket.on('message_deleted', (data) => {
    const el = document.getElementById(`msg-${data.msgId}`);
    if (el) {
        const midia = el.querySelector('.chat-image, .chat-video, .chat-audio, .chat-pdf'); if(midia) midia.remove();
        ...
        if (textSpan) { textSpan.innerHTML = apagadaHtml; } else { const infoDiv = el.querySelector('div[style*="font-size:12.5px"]'); const statusDiv = el.querySelector('div[style*="position: absolute"]'); el.innerHTML = (infoDiv ? infoDiv.outerHTML : '') + apagadaHtml + (statusDiv ? statusDiv.outerHTML : ''); }
    }
...
// 3. Editar o Nome do Grupo
window.editGroupName = async function(groupId, currentName) {
    const safeName = (currentName === 'undefined' || currentName === 'null') ? '' : currentName;
    const newName = prompt("Digite o novo nome do grupo:", safeName);
    
    if (newName !== null && newName.trim() !== "" && newName.trim() !== safeName) {
        try {
            const res = await fetch(`/groups/${groupId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (res.ok) {
                window.showCurrentChatProfile(); // Atualiza a tela de perfil do grupo
                const chatTitle = document.getElementById('chat-title');
                if(chatTitle) chatTitle.innerText = newName.trim(); // Atualiza o topo do chat
                if (socket) socket.emit('group_updated'); // Pede para os outros contatos atualizarem
            } else { alert("Erro ao atualizar nome do grupo no servidor."); }
        } catch (e) { alert("Erro de conexão."); }
    }
};

// 4. Ver Foto de Perfil Expandida (Grupos e Contatos)
window.viewGroupPhoto = function(photoUrl) {
    const viewerImg = document.getElementById('viewer-photo');
...
// 5. BÔNUS: Enviar Nova Foto do Grupo para o Banco de Dados
window.uploadAndUpdateGroupPhoto = async function(groupId, input) {
    const file = input.files[0];
    if(!file) return;
    
    // Mostra spinner na tela
    const label = document.querySelector('label[for="edit-group-photo-input"]');
    if(label) label.innerHTML = '<span class="material-icons-round" style="color:white; animation: spin 1s infinite;">sync</span>';
    ...
    try {
        ...
        if(uploadData.url) {
            ...
            if(chatAvatar) chatAvatar.src = uploadData.url; 
            if (socket) socket.emit('group_updated');
        }
    } catch(e) {
        alert("Erro ao enviar a nova foto.");
    } finally {
        if(label) label.innerHTML = '<span class="material-icons-round" style="color:white;">photo_camera</span>';
        input.value = '';
    }
};

// 6. Remover Membro do Grupo (Apenas para o Admin)
window.removeGroupMember = async function(groupId, memberId, memberName) {
    if (!confirm(`⚠️ Tem certeza que deseja remover ${memberName} do grupo?`)) return;
    
    try {
        const res = await fetch(`/groups/${groupId}/remove-members`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
            window.showCurrentChatProfile(); // Atualiza a lista na tela imediatamente
            if (socket) socket.emit('group_updated'); // Pede para todos os dispositivos atualizarem
            alert(`✅ ${memberName} foi removido do grupo.`);
        } else { 
            alert("Erro ao remover o membro no servidor."); 
        }
    } catch (e) { 
        alert("Erro de conexão."); 
    }
...
// Garante que o botão de abrir/fechar a barra funciona sempre
...
// Motor de Busca Blindado (Com Debounce Anti-Crash)
window.handleSearch = function(query) {
    ...
    // Aguarda 600ms antes de pesquisar (Impede sobrecarga do servidor)
        ...
        try {
            ...
            if (data.users && data.users.length > 0) {
                ...
                data.users.forEach(u => {
                    list.innerHTML += `
                        <div class="user-item" onclick="openChat('${u._id}', '${(u.displayName || u.email).replace(/'/g, "\\'")}', '${u.photoUrl}', '${u.email}', 'user'); toggleMainSearch();">
                            <div class="user-avatar-container"><img src="${u.photoUrl || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'}"></div>
            ...
            // 2. RENDERIZAR MENSAGENS (HIGHLIGHT E CONTAS FANTASMA CORRIGIDO)
            if (data.messages && data.messages.length > 0) {
                ...
                data.messages.forEach(m => {
                    ...
                    if (m.groupId) {
                        ...
                        const receiverId = m.receiver && m.receiver._id ? String(m.receiver._id) : String(m.receiver);
                        
                        const other = (senderId === myIdStr) ? m.receiver : m.sender;
                        if(other && other._id) {
                            targetId = String(other._id);
                            chatName = other.displayName || other.email || 'Usuário';
                            chatPhoto = other.photoUrl || chatPhoto;
                        } else {
                            targetId = (senderId === myIdStr) ? receiverId : senderId;
                        }
                    }
                    ...
                    if (m.sender) {
                        ...
                        senderName = (sid === String(myId)) ? 'Você' : (m.sender.displayName || 'Alguém');
                    }
...
// PREPARA O PULO MÁGICO (Assinatura corrigida para abrir o Chat)
window.jumpToMessage = function(targetId, chatName, chatPhoto, isGroup, msgId, searchWord) {
    ...
    if(typeof openChat === 'function') {
        openChat(targetId, chatName, chatPhoto, '', isGroup ? 'group' : 'user');
    }
};

// O RASTREADOR: Fica escondido vendo as mensagens entrarem na tela
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('.targeted-msg-active')) {
        ...
        activeMsgs.forEach(msg => {
            msg.classList.remove('targeted-msg-active'); 
