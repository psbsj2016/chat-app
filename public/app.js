...
function showMainScreen() { hideElement('auth-screen'); hideElement('welcome-screen'); hideElement('permissions-screen'); hideAllTabs(); showElement('main-screen'); loadContacts(); socket.emit('join_room', myId); if ("Notification" in window && Notification.permission === "granted") registerServiceWorkerAndSubscribe(); const navItems = document.querySelectorAll('.nav-item'); if(navItems.length > 0) navItems[0].classList.add('active'); }
...
// ==============================================================
// 📝 MOTOR DE ANOTAÇÕES (Integrado de volta)
// ==============================================================
let currentNotes = []; 
...
// ==============================================================
function openImmersiveGame(gameUrl, gameTitle) { const modal = document.getElementById('game-immersive-modal'); const iframe = document.getElementById('immersive-game-frame'); const title = document.getElementById('immersive-game-title'); title.innerText = gameTitle.toUpperCase(); iframe.src = gameUrl; modal.classList.remove('hidden'); const nav = document.getElementById('bottom-navigation'); if(nav) nav.style.display = 'none'; }
function closeImmersiveGame() { const modal = document.getElementById('game-immersive-modal'); const iframe = document.getElementById('immersive-game-frame'); iframe.src = ''; modal.classList.add('hidden'); const nav = document.getElementById('bottom-navigation'); if(nav) nav.style.display = 'flex'; }
...
window.fetchAndSyncProfile = async function() { const todayStr = new Date().toISOString().split('T')[0]; if (cachedMe.lastActiveDate !== todayStr) { cachedMe.dailyMessagesSent = 0; cachedMe.dailyMissionCompleted = false; } renderDailyMission(cachedMe.dailyMessagesSent || 0, cachedMe.dailyMissionCompleted || false); };
        ...
        if (res.ok) { 
            if (isRegistering) { 
                ...
                if (localStorage.getItem('isFirstLogin') === 'true') { 
                    localStorage.removeItem('isFirstLogin'); showWelcomeScreen(); 
                } else { 
                    checkAndShowPermissions(); 
                } 
            } 
        } else { 
            alert(data.error || 'Erro na autenticação. Verifique os dados.'); 
        } 
    } catch (e) { 
        alert("🚨 O servidor recusou a conexão. Verifique a internet e tente novamente."); 
    } finally { 
