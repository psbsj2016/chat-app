...
function backToMain() {
    ...
    hideElement('chat-screen');
    showElement('main-screen');
    loadContacts(); // Recarrega para atualizar ordem caso haja novas mensagens
}

// ==========================================
// 2. SOCKETS: TEMPO REAL, ONLINE E MENSAGENS
// ==========================================
...
if (msgInput) {
    msgInput.addEventListener('input', () => {
        ...
        typingTimeout = setTimeout(() => {
            socket.emit('stop_typing', { senderId: myId, receiverId: currentChatId });
        }, 1500);
    });
}
...
// RECEBER MENSAGENS EM TEMPO REAL E DESTACAR CONTATO
socket.on('receive_message', (msg) => {
    const senderIdStr = (typeof msg.sender === 'object') ? msg.sender._id : msg.sender;
    
    // Se o chat com a pessoa/grupo estiver ABERTO
    if (isGroupChat && msg.groupId === currentChatId) {
        displayMessage(msg);
    } else if (!isGroupChat && (senderIdStr === myId || (senderIdStr === currentChatId && msg.receiver === myId))) {
        displayMessage(msg);
        
        // Se eu recebi e o chat ta aberto, já marco como lido no banco
        if(senderIdStr === currentChatId) {
            fetch('/messages/mark-read', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ myId, otherId: currentChatId }) });
        }
    } else {
        // MENSAGEM NOVA DE ALGUÉM QUE NÃO ESTÁ ABERTO (MÁGICA AQUI!)
        const targetId = msg.groupId ? msg.groupId : senderIdStr;
        const contactDiv = document.getElementById(`contact-${targetId}`);
        
        if (contactDiv) {
            contactDiv.classList.add('has-unread'); // Pinta de verde
            
            // Joga o contato para o topo da lista!
...
function openChat(id, name, photo, email, type = 'user') {
    ...
    // Tira a cor de "não lido" da tela inicial se existir
    const contactDiv = document.getElementById(`contact-${id}`);
    if (contactDiv) contactDiv.classList.remove('has-unread');

    // Se for mensagem de um usuário, avisa o banco de dados que leu
    if (!isGroupChat) {
        fetch('/messages/mark-read', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ myId, otherId: id })
        });
    }
    ...
    if (headerDot) {
        if (isGroupChat) {
            ...
            headerDot.className = `status-dot ${onlineUsersList.includes(id) ? 'status-online' : 'status-offline'}`;
        }
    }
...
async function loadContacts() {
    ...
    // 1. Busca quem me mandou mensagem que eu não li
    let unreadSenders = [];
    try {
        const resUnread = await fetch(`/unread/${myId}`);
        unreadSenders = await resUnread.json();
    } catch(e) {}
    ...
    groups.forEach(group => {
        ...
        div.id = `contact-${group._id}`; // ID pra achar depois
    ...
    // 3. Contatos Individuais
    const resUsers = await fetch(`/users/${myId}`);
    const users = await resUsers.json();

    // Organizar: Quem tem mensagem não lida vai pro topo
    ...
    users.forEach(user => {
        ...
        let sectorLabel = '';
        let extraClass = unreadSenders.includes(user._id) ? 'has-unread' : ''; // Aplica a classe se não lido
        ...
        div.id = `contact-${user._id}`; // ID pra achar depois
        ...
        clickArea.innerHTML = `
            ...
            <div class="info">
                ...
                <div style="font-size:12px; color:var(--secondary-text)">${unreadSenders.includes(user._id) ? 'Nova mensagem!' : 'Toque para conversar'}</div>
            </div>
        `;
...
async function deleteCurrentChat() {
    ...
    if (!confirm("⚠️ ATENÇÃO!\nTem certeza que deseja apagar TODA a conversa?")) return;
...
async function handleFileUpload(input) {
    ...
    const btn = document.querySelector('.send-btn'); btn.innerHTML = '<span class="material-icons">sync</span>'; 
...
function openAddSectorModal(userId, name) {
    targetContactId = userId; hideElement(`contact-menu-${userId}`);
    document.getElementById('sector-target-name').innerText = `Contato: ${name}`;
    const list = document.getElementById('sector-checkbox-list'); list.innerHTML = '';
    if(currentSectors.length === 0) list.innerHTML = '<span style="font-size:12px; color:#999;">Nenhum setor criado.</span>';
    currentSectors.forEach((sec, idx) => {
        const isAlreadyIn = sec.members.includes(userId);
        list.innerHTML += `<label class="checkbox-item"><input type="checkbox" value="${idx}" ${isAlreadyIn ? 'checked disabled' : ''}> ${sec.name} ${isAlreadyIn ? '(Já no setor)' : ''}</label>`;
    });
...
async function submitAddSector() {
    ...
    if(!confirm("O contato será inserido. Deseja continuar?")) return;
    checkboxes.forEach(cb => currentSectors[cb.value].members.push(targetContactId));
    await saveProfile({ sectors: currentSectors });
    alert("Contato inserido no setor!"); hideElement('add-sector-modal'); loadContacts(); 
}

async function openAddGroupModal(userId, name) {
    targetContactId = userId; hideElement(`contact-menu-${userId}`);
    document.getElementById('group-target-name').innerText = `Contato: ${name}`;
    const res = await fetch(`/groups/${myId}`); const groups = await res.json();
    const list = document.getElementById('group-checkbox-list'); list.innerHTML = '';
    if(groups.length === 0) list.innerHTML = '<span style="font-size:12px; color:#999;">Você não tem grupos.</span>';
    groups.forEach((g) => {
        const isAlreadyIn = g.members.includes(userId);
        list.innerHTML += `<label class="checkbox-item"><input type="checkbox" value="${g._id}" ${isAlreadyIn ? 'checked disabled' : ''}> ${g.name} ${isAlreadyIn ? '(Já no grupo)' : ''}</label>`;
    });
...
async function submitAddGroup() {
    ...
    if(!confirm("O contato participará do(s) chat(s). Clique em OK para adicionar.")) return;
    try {
        const res = await fetch('/groups/add-member', { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ groupIds, userId: targetContactId }) });
        if(res.ok) { alert("Contato inserido no chat do grupo!"); hideElement('add-group-modal'); }
    } catch(e) { alert("Erro ao adicionar"); }
}

async function openCreateGroupModal() {
    toggleMenu('main-menu'); showElement('create-group-modal'); selectedUserIds = []; document.getElementById('group-name-input').value = '';
    ...
    users.forEach(user => {
        ...
        div.onclick = () => {
            if (selectedUserIds.includes(user._id)) { selectedUserIds = selectedUserIds.filter(uid => uid !== user._id); div.classList.remove('selected'); } 
            else { selectedUserIds.push(user._id); div.classList.add('selected'); }
        };
        const photo = user.photoUrl || 'https://cdn-icons-png.flaticon.com/512/149/149071.png';
        div.innerHTML = `<img src="${photo}"><span>${user.displayName || user.email}</span>`; list.appendChild(div);
    });
}
...
async function submitCreateGroup() {
    ...
    try {
        const res = await fetch('/groups', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, adminId: myId, members: selectedUserIds }) });
        if (res.ok) { alert("Grupo criado!"); closeCreateGroup(); loadContacts(); }
    } catch (e) {}
}
...
function openSettings() {
    ...
    document.getElementById('theme-switch').checked = localStorage.getItem('theme') === 'dark'; renderSectorsList();
}
...
function renderSectorsList() {
    ...
    currentSectors.forEach(sec => {
        const div = document.createElement('div'); div.className = 'setting-item'; div.innerHTML = `<span>${sec.name}</span> <small>${sec.members.length} membros</small>`; list.appendChild(div);
    });
}
...
async function uploadProfilePhoto(input) {
    const file = input.files[0]; if(!file) return; const formData = new FormData(); formData.append('file', file);
    ...
    try {
        ...
        if(avatar) { avatar.src = data.url; avatar.style.opacity = '1'; } saveProfile({ photoUrl: data.url });
    } catch (e) { alert('Erro no upload'); }
}
...
function deleteAccount() { if(confirm("TEM CERTEZA? Isso apagará tudo!")) { fetch(`/delete-account/${myId}`, { method: 'DELETE' }).then(() => { alert("Conta deletada."); logout(); }); } }
...
function renderSearchResults(data) {
    ...
    if (data.users.length > 0) { list.innerHTML += '<div class="search-section-title">Contatos</div>'; data.users.forEach(user => list.appendChild(createSearchItem(user, null))); }
    if (data.messages.length > 0) { list.innerHTML += '<div class="search-section-title">Mensagens</div>'; data.messages.forEach(msg => { const isMeSender = msg.sender._id === myId; const chatPartner = isMeSender ? msg.receiver : msg.sender; list.appendChild(createSearchItem(chatPartner, msg)); }); }
...
function createSearchItem(user, msgMatch) {
    const div = document.createElement('div'); div.className = 'user-item'; div.onclick = () => openChat(user._id, user.displayName, user.photoUrl, user.email, 'user');
    const photo = user.photoUrl || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'; const name = user.displayName || 'Usuário'; let subText = 'Toque para conversar';
    if (msgMatch) subText = `<span style="color:#008069">Encontrado:</span> "${msgMatch.content}"`;
    div.innerHTML = `<img src="${photo}" alt="Avatar"><div class="info"><div style="font-weight:bold">${name}</div><div class="match-preview">${subText}</div></div>`; return div;
}
...
async function handleAuth() {
    const email = document.getElementById('auth-email').value; const password = document.getElementById('auth-pass').value; const name = document.getElementById('auth-name').value;
    const btn = document.getElementById('auth-btn'); if (!email || !password) return alert("Preencha todos os campos!");
    const textoOriginal = btn.innerText; btn.innerText = "Processando..."; btn.disabled = true;
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            if (isRegistering) { alert('✅ Código enviado!'); const code = prompt("Digite o código recebido:"); if(code) verifyCodeManual(email, code); } 
            else {
        ...
        } else { alert('Erro: ' + (data.error || 'Algo deu errado')); }
    } catch (e) { alert('Erro de conexão.'); } finally { btn.innerText = textoOriginal; btn.disabled = false; }
}
...
async function verifyCodeManual(email, code) {
    try {
        ...
        if(res.ok) { alert("Verificado!"); toggleAuthMode(); } else { alert("Código errado."); }
    } catch(e) {}
}
    ...
    if(token && myId) { 
        try {
            ...
            if(res.ok) {
                ...
                if (me.theme === 'dark') document.body.classList.add('dark-mode'); else document.body.classList.remove('dark-mode');
            }
        } catch(e) {}
        showMainScreen(); 
