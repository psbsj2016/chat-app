...
function renderContactsList(groups, users) {
    ...
    visibleUsers.forEach(user => { 
        ...
        const menuArea = document.createElement('div'); menuArea.className = 'contact-actions'; menuArea.onclick = (e) => { e.stopPropagation(); toggleMenu(`contact-menu-${user._id}`); }; const sectorBtnText = 'Classificar Contato'; 
...
function backToSettings() { 
    ...
    hideElement('classifications-screen');
...
function renderClassificationsList() { const list = document.getElementById('classifications-list'); list.innerHTML = ''; if(currentSectors.length === 0) { list.innerHTML = '<div style="text-align:center; color:var(--secondary-text); padding:20px;">Nenhuma classificação criada.</div>'; return; } const cachedUsers = JSON.parse(localStorage.getItem('cacheUsers')) || []; currentSectors.forEach((sec, sIdx) => { let membersHtml = ''; if(sec.members.length === 0) { membersHtml = '<div style="padding: 10px 15px; color: var(--secondary-text); font-size: 13px;">Nenhum contato classificado aqui</div>'; } else { sec.members.forEach(memberId => { const u = cachedUsers.find(user => user._id === memberId); if(u) { membersHtml += `<div style="padding: 10px 15px; display:flex; align-items:center; gap:10px; border-bottom: 1px solid var(--input-bg);"><img src="${u.photoUrl || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'}" style="width:30px; height:30px; border-radius:50%; object-fit:cover;"> <span style="font-size: 14px; font-weight:600; color: var(--text-color);">${u.displayName || u.email}</span></div>`; } }); } list.innerHTML += `<div class="settings-group" style="margin-bottom: 15px;"><div style="padding: 15px; background: var(--input-bg); display:flex; justify-content:space-between; align-items:center; font-weight: 800; color: var(--brand-primary);">${sec.name} <span class="material-icons-round" style="color:#EF4444; font-size:20px; cursor:pointer;" onclick="deleteClassification(${sIdx})">delete</span></div>${membersHtml}</div>`; }); }

function deleteClassification(index) { if(confirm('Excluir esta classificação? Os contatos não serão apagados, apenas perderão a etiqueta.')) { currentSectors.splice(index, 1); renderClassificationsList(); saveProfile({ sectors: currentSectors }); loadContacts(); } }
...
// ==============================================================
// 🎙️ MOTOR WEBRTC: LOUNGE DE VOZ EM TEMPO REAL (MESH MULTI-PEER)
// ==============================================================
...
function leaveVoiceChannel() {
    ...
    // Destrói todos os túneis de quem estava conectado a você
...
// Quando um novo membro entra na sala, VOCÊ cria um túnel e liga para ele
socket.on('user_joined_voice', async (data) => {
    const peerSocketId = data.socketId;
    if(peerConnections[peerSocketId]) return; // Evita eco duplicado
...
// Recebendo e processando os cabos de rede (Sinais)
...
// Desconectando pessoas individuais
...
function createPeerConnection(socketId, userProfile) {
    ...
    pc.onicecandidate = (event) => {
        if (event.candidate) {
            socket.emit('webrtc_signal', { to: socketId, signal: { candidate: event.candidate } });
        }
    };
    ...
    pc.ontrack = (event) => {
        ...
        // Força a reprodução no celular
        audioElement.play().catch(e => console.log("Áudio bloqueado pelo Safari/Chrome até interação do usuário."));
    };

    return pc;
}

// UI: Mostra o Avatar na tela
