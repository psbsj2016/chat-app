...
// ==========================================
// 2. SOCKETS: TEMPO REAL, ONLINE E DIGITANDO
// ==========================================
socket.on('online_users', (list) => {
    ...
    // B. Atualiza a bolinha do cabeçalho do chat (se estiver aberto)
    if (currentChatId && !isGroupChat) {
        const headerDot = document.getElementById('chat-header-status');
        if (headerDot) {
            headerDot.className = `status-dot ${onlineUsersList.includes(currentChatId) ? 'status-online' : 'status-offline'}`;
        }
    }
});

// Detecta quando EU estou digitando
...
// Ouve quando O OUTRO está digitando
socket.on('typing', (data) => {
    if (data.senderId === currentChatId && !isGroupChat) {
...
function openChat(id, name, photo, email, type = 'user') {
    ...
    hideElement('typing-indicator'); // Esconde o digitando ao abrir
...
// ==========================================
// 5. SETORES, GRUPOS E CONFIGURAÇÕES
// ==========================================
...
async function initApp() {
    if(token && myId) { 
        ...
        } catch(e) { console.error("Erro na inicialização"); }
        showMainScreen(); 
    } else { 
